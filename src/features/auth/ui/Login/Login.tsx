import { selectThemeMode, setIsCaptchaAC, setIsLoggedInAC } from "@/app/app-slice"
...
export const Login = () => {
  ...
  const [captchaData, { data }] = useLazyCaptchaQuery()
  ...
  const onSubmit: SubmitHandler<LoginInputs> = (data) => {
    login({ ...data }).then((res) => {
      ...
      if (res.data?.resultCode === ResultCode.CaptchaError) {
        captchaData()
        dispatch(setIsCaptchaAC({ captcha: true }))
        reset()
      }
    })
  }
  const images = data ? data.url : undefined
  return (
    <>
            ...
            <FormGroup>
              ...
              {data && (
                <div style={{ display: "flex", flexDirection: "row", marginBottom: "10px" }}>
                  ...
                  >
                    <img src={images} alt={"12"} width={300} height={100} />
                    <button
                      type={"button"}
                      style={{ width: "150px", backgroundColor: "lawngreen" }}
                      onClick={() => captchaData()}
                    >
                      captcha refresh
                    </button>
